...
| **Gradio dropdown error during generation** | Your ACE-Step version's `/generation_wrapper` schema has drifted from this UI — the endpoint is positional, so an inserted or removed control shifts every later value into the wrong slot and dropdowns reject what they receive. Compare `buildGradioArgs` in `server/src/services/acestep.ts` against the `inputs=[...]` list in ACE-Step's `acestep/ui/gradio/events/wiring/generation_run_wiring.py`. |
